...
// 辅助函数：获取测试配置
func getTestDeepSeekConfig() providerClientOptions {
	// 使用提供的 API 配置
...
// TestDeepSeekBasicSetup 测试基础设置
func TestDeepSeekBasicSetup(t *testing.T) {
	if testing.Short() {
		t.Skip("跳过集成测试")
	}
	...
	opts := getTestDeepSeekConfig()
	client := newOpenAIClient(opts)

	if client == nil {
		t.Fatal("Failed to create DeepSeek client")
	}

	model := client.Model()
	if model.ID != "DeepSeek-V3" {
		t.Errorf("Expected model ID to be DeepSeek-V3, got %s", model.ID)
	}

	t.Logf("Successfully created DeepSeek client with model: %s", model.Name)
}

// TestDeepSeekSend 测试同步发送消息
...
// TestDeepSeekContextCancellation 测试上下文取消
func TestDeepSeekContextCancellation(t *testing.T) {
	if testing.Short() {
		t.Skip("跳过集成测试")
	}
...
// TestDeepSeekMultiTurn 测试多轮对话
func TestDeepSeekMultiTurn(t *testing.T) {
	if testing.Short() {
		t.Skip("跳过集成测试")
	}
...
// mockDeepSeekTool 实现 tools.BaseTool 接口用于测试
type mockDeepSeekTool struct {
	name        string
	description string
	parameters  map[string]any
	required    []string
...
func (m *mockDeepSeekTool) Info() tools.ToolInfo {
...
func (m *mockDeepSeekTool) Name() string {
	return m.name
}

func (m *mockDeepSeekTool) Run(_ context.Context, _ tools.ToolCall) (tools.ToolResponse, error) {
