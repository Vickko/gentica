...
func TestAgentWithTODO(t *testing.T) {
	// 创建临时工作目录
	...
	todoDir, err := mgr.Add("TODO", "Task management directory for tracking progress. Use TODO.md file with GitHub-style task lists (- [ ] for pending, - [x] for completed). "+
		"Always plan for composite task and make TODO list before task and update TODO.md after completing each major step. "+
		"You MUST ONLY write TODO list itself into the file and NOTHING MORE.")
	require.NoError(t, err)
	...
	agent := NewBuilder(
		g,
		"chef_assistant",
		"Autonomous chef preparing dinner",
		`You are a professional chef who must prepare an Italian pasta dinner for 4 people.

IMPORTANT RULES:
- You MUST plan before taking action, use TODO list to organize your task
- You can find the TODO directory by using resource_directory_list tool
- Think how your cooking process would be, and simulate it step by step realistically (e.g., "Now I'm chopping the onions...")
- You SHOULD state what you are doing before updating TODO, and that will be considered completion of the task. 
- After EACH simulated step, update TODO.md to mark that task as complete
- Work completely autonomously until complete the ENTIRE dinner preparation process, don't wait for further instructions

Example TODO.md format:
# Italian Pasta Dinner
- [x] Gather ingredients
- [x] Prepare vegetables
- [ ] Make marinara sauce
- [ ] Cook pasta
- [ ] Set table
- [ ] Plate and serve

Begin now and complete the entire dinner preparation autonomously.`,
	).WithTools(genkitTools...).
		WithModel("openai/gpt-5-mini").
		WithTemperature(0.1).
		WithMaxRounds(50). // 大幅增加回合数
		WithLogging(true).
		Build()

	ctx := context.Background()

	// 单一调用 - 让 Agent 自主完成整个流程
	t.Logf("Starting autonomous chef agent...")
	result, err := agent.Run(ctx, "Prepare an Italian pasta dinner for 4 people. Plan it, cook it step by step (simulated), and track all progress in TODO. Complete the entire process autonomously.")
	require.NoError(t, err, "Agent should complete the task")
	require.NotEmpty(t, result, "Agent should provide a response")

	t.Logf("Agent completed with result:\n%s", result)

	// 最终验证 - 检查 Agent 是否成功完成了整个流程
	todoMdPath := filepath.Join(todoDir.Path, "TODO.md")

	// 验证 TODO.md 文件存在
	_, err = os.Stat(todoMdPath)
	require.NoError(t, err, "TODO.md should be created")
	...
	t.Logf("Final task status - Completed: %d, Pending: %d", completedCount, pendingCount)
	assert.Greater(t, completedCount, 0, "Should have at least some completed tasks")

	// 验证包含关键的烹饪相关内容
	todoLower := strings.ToLower(todoContent)
	hasRelevantContent := strings.Contains(todoLower, "pasta") ||
		strings.Contains(todoLower, "sauce") ||
		strings.Contains(todoLower, "ingredients") ||
		strings.Contains(todoLower, "cook")
	assert.True(t, hasRelevantContent, "TODO.md should contain cooking-related tasks")

	// 验证有进度（至少有一些任务从未完成变为完成）
	if completedCount > 0 {
		t.Logf("✅ Chef successfully tracked progress: %d tasks completed", completedCount)
	}

	// 最终总结
	t.Logf("========== Test Summary ==========")
	t.Logf("✅ Agent worked autonomously")
	t.Logf("✅ TODO.md was created and maintained")
	t.Logf("✅ GitHub-style task lists were used")
	t.Logf("✅ Progress was tracked (%d tasks completed)", completedCount)
	t.Logf("==================================")
}
