...
func (t *sourcegraphTool) Run(ctx context.Context, call ToolCall) (ToolResponse, error) {
	...
	} else if params.ContextWindow > 50 {
		params.ContextWindow = 50 // Reasonable max to avoid huge outputs
	}
	...
	patternType := "standard" // Default to standard for better regex and literal support
	if strings.Contains(params.Query, "literal:") || strings.Contains(params.Query, "content:") {
		patternType = "literal"
	}

	request := graphqlRequest{
		Query: "query Search($query: String!) { search(query: $query, version: V3, patternType: " + patternType + ") { results { matchCount, limitHit, resultCount, approximateResultCount, missing { name }, timedout { name }, indexUnavailable, results { __typename, ... on FileMatch { repository { name }, file { path, url, content }, lineMatches { preview, lineNumber, offsetAndLengths } } } } } }",
	}
	...
	req.Header.Set("User-Agent", "gentica-sourcegraph-tool/1.0")
	...
	if resp.StatusCode != http.StatusOK {
		...
		if len(body) > 0 {
			return NewTextErrorResponse(fmt.Sprintf("Sourcegraph API request failed with status %d: %s", resp.StatusCode, string(body))), nil
		}

		return NewTextErrorResponse(fmt.Sprintf("Sourcegraph API request failed with status code: %d", resp.StatusCode)), nil
	}
...
func formatSourcegraphResults(result map[string]any, contextWindow int, count int) (string, error) {
	if result == nil {
		return "No results to format", nil
	}
	...
	// Respect the count parameter passed in
	maxResults := count
	if maxResults <= 0 {
		maxResults = 10
	...
	for i, res := range results {
		...
		if len(lineMatches) > 0 {
			// Limit line matches to avoid excessive output
			maxLineMatches := 5
			if len(lineMatches) > maxLineMatches {
				lineMatches = lineMatches[:maxLineMatches]
				buffer.WriteString(fmt.Sprintf("(Showing first %d matches)\n\n", maxLineMatches))
			}
			...
			for _, lm := range lineMatches {
				...
				// Skip invalid line matches
				if lineNumber == 0 && preview == "" {
					continue
				}
				...
				if fileContent != "" {
					...
					endLine := min(len(lines), int(lineNumber)+contextWindow)
					...
					// Show the matching line with emphasis
