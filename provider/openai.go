...
func newOpenAIClient(config ProviderConfig, opts *ProviderOptions) Provider {
	return &openaiClient{
		config: config,
		opts:   opts,
...
func createOpenAIClient(config ProviderConfig) openai.Client {
	openaiClientOptions := []option.RequestOption{}
	if config.APIKey != "" {
		openaiClientOptions = append(openaiClientOptions, option.WithAPIKey(config.APIKey))
	}
	if config.BaseURL != "" {
		openaiClientOptions = append(openaiClientOptions, option.WithBaseURL(config.BaseURL))
	}
	...
	for extraKey, extraValue := range config.ExtraBody {
...
func (o *openaiClient) convertMessages(messages []message.Message) (openaiMessages []openai.ChatCompletionMessageParamUnion) {
	isAnthropicModel := strings.HasPrefix(o.config.Model, "anthropic/")
	// Add system message first
	systemMessage := ""
	if o.opts != nil {
		systemMessage = o.opts.SystemMessage
	}
	if o.config.SystemPromptPrefix != "" {
		systemMessage = o.config.SystemPromptPrefix + "\n" + systemMessage
	}

	system := openai.SystemMessage(systemMessage)
	if isAnthropicModel && (o.opts == nil || !o.opts.DisableCache) {
	...
	for i, msg := range messages {
		...
		case message.User:
			...
			for _, binaryContent := range msg.BinaryContent() {
				...
				imageURL := openai.ChatCompletionContentPartImageImageURLParam{URL: binaryContent.String()}
				imageBlock := openai.ChatCompletionContentPartImageParam{ImageURL: imageURL}

				content = append(content, openai.ChatCompletionContentPartUnionParam{OfImageURL: &imageBlock})
			}
			if cache && (o.opts == nil || !o.opts.DisableCache) && isAnthropicModel {
			...
			if hasBinaryContent || (isAnthropicModel && (o.opts == nil || !o.opts.DisableCache)) {
...
func (o *openaiClient) preparedParams(messages []openai.ChatCompletionMessageParamUnion, tools []openai.ChatCompletionToolParam) openai.ChatCompletionNewParams {
	params := openai.ChatCompletionNewParams{
		Model:    openai.ChatModel(o.config.Model),
		Messages: messages,
		Tools:    tools,
	}

	// Check if this is a reasoning model
	if strings.Contains(o.config.Model, "o1") || strings.Contains(o.config.Model, "reasoning") {
		// 只有当明确设置了 MaxTokens 时才设置 MaxCompletionTokens
		if o.config.MaxTokens > 0 {
			params.MaxCompletionTokens = openai.Int(o.config.MaxTokens)
		}
		if o.config.ReasoningEffort != "" {
			switch o.config.ReasoningEffort {
			case "low":
		...
		// 只有当明确设置了 MaxTokens 时才设置 MaxTokens
		if o.config.MaxTokens > 0 {
			params.MaxTokens = openai.Int(o.config.MaxTokens)
		}
	}
...
func (o *openaiClient) shouldRetry(attempts int, err error) (bool, int64, error) {
	...
	if errors.As(err, &apiErr) {
		...
		if apiErr.StatusCode == 401 {
			// Simply return the authentication error
...
func (o *openaiClient) Model() Model {
	// Return a simple model based on config
	return Model{
		ID:               o.config.Model,
		Name:             o.config.Model,
		DefaultMaxTokens: o.config.MaxTokens,
	}
}
