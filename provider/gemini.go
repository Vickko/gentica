...
func newGeminiClient(config ProviderConfig, opts *ProviderOptions) Provider {
	fmt.Printf("DEBUG newGeminiClient: Creating client with config: %+v\n", config)
	client, err := createGeminiClient(config)
	if err != nil {
		slog.Error("Failed to create Gemini client", "error", err)
		fmt.Printf("DEBUG newGeminiClient: ERROR creating client: %v\n", err)
		return nil
	}
	fmt.Printf("DEBUG newGeminiClient: Client created successfully: %v\n", client != nil)
...
func createGeminiClient(config ProviderConfig) (*genai.Client, error) {
	// 如果有自定义的 baseURL，设置它
	if config.BaseURL != "" {
...
func (g *geminiClient) Send(ctx context.Context, messages []message.Message, tools []tools.BaseTool) (*ProviderResponse, error) {
	...
	systemMessage := ""
	if g.opts != nil {
		systemMessage = g.opts.SystemMessage
	}
	if g.config.SystemPromptPrefix != "" {
		systemMessage = g.config.SystemPromptPrefix + "\n" + systemMessage
	}
	...
	// 只有当明确设置了 MaxTokens 时才设置 MaxOutputTokens
	if g.config.MaxTokens > 0 {
		config.MaxOutputTokens = int32(g.config.MaxTokens)
	}
	config.Tools = g.convertTools(tools)
	chat, _ := g.client.Chats.Create(ctx, g.config.Model, config, history)
...
func (g *geminiClient) shouldRetry(attempts int, err error) (bool, int64, error) {
	...
	if contains(errMsg, "unauthorized", "invalid api key", "api key expired") {
		...
		return false, 0, fmt.Errorf("invalid or expired API key")
	}
...
func (g *geminiClient) Model() Model {
	// Return a simple model based on config
	return Model{
		ID:               g.config.Model,
		Name:             g.config.Model,
		DefaultMaxTokens: g.config.MaxTokens,
	}
}
